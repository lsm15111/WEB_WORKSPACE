...
function App() {
  const [saramList, setSaramList] = useState([
    {no:1, irum:"Sample", tel:"010-1111-1111"}
  ]);
  const [irum, setIrum] = useState('김범준');
  ...
  return (
    <div>
      ...
      <fieldset>
        이름 <input type="text" name="irum" value={irum} onChange={e=>setIrum(e.currentTarget.value)}/><br/>
        연락처 <input type="text" name="irum" value={tel} onChange={e=>setTel(e.currentTarget.value)}/><br/>
        <button onClick={(event) =>{
          ...
          let newSaram = {no: maxNo+1, irum, tel};
